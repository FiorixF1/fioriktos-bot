...
CHATS         = dict()      # key = chat_id [integer] --- value = Chat [object]
DISK_CHATS    = set()       # chats in local storage: set of chat_key [string]
NETWORK_CHATS = set()       # chats in remote storage: set of chat_key [string]
...
def chat_finder(f):
    ...
    def wrapped(bot, update, *args, **kwargs):
        ...
        if chat_id in CHATS:
            chat = CHATS[chat_id]
        elif chat_key in DISK_CHATS:
            with open(chat_key, "r") as dump:
                data = dump.read()
            chat = unjsonify(data)
            CHATS[chat_id] = chat
        elif chat_key in NETWORK_CHATS:
