...
""" This class handles data storage and synchronization (Local ThreeLevelCache edition) """
class LocalThreeLevelCache:
    def __init__(self, logger):
        ...
        self.CHAT_UPDATE_TIMEOUT   = 3600      # 1 hour
            ...
            for page in pages:
                for chat_aws in page["Contents"]:
                    if now - chat_aws["LastModified"].timestamp() > self.CHAT_MAX_DURATION and chat_aws["Key"] != self.PREFIX:
                        # TODO: no need to remove from local storage: in Heroku it is freed at boot
